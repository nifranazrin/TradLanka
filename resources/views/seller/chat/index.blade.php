...
<div class="chat-wrapper">
    <div class="chat-container">
        <div class="chat-sidebar" id="sidebarContainer">
            <div class="chat-header"><span>Seller Messages</span></div>
            ...
            <div class="contact-list" id="contactList">
                ...
                <div class="contact-item {{ request('id') == $admin->id ? 'active' : '' }}" id="contact-admin-{{ $admin->id }}" onclick="selectContact({{ $admin->id }}, 'admin')">
                    <div class="avatar-wrapper">
                        <img src="{{ $admin->image ? asset('storage/'.$admin->image) : 'https://ui-avatars.com/api/?name=Admin&background=6b2f2f&color=fff' }}" 
                ...
                <div class="contact-item" id="contact-{{ $contact->role }}-{{ $contact->id }}" onclick="selectContact({{ $contact->id }}, '{{ $contact->role }}')">
                    <div class="avatar-wrapper">
                        <img src="{{ $contact->image ? asset('storage/'.$contact->image) : 'https://ui-avatars.com/api/?name='.urlencode($contact->name).'&background=random' }}" 
                             onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name={{ urlencode($contact->name) }}&background=random';" class="contact-avatar">
                    </div>
                    <div class="flex-grow-1">
                ...
                @if(isset($allOtherStaff))
                    ...
                    <div class="contact-item new-contact" id="contact-{{ $staff->role }}-{{ $staff->id }}" onclick="selectContact({{ $staff->id }}, '{{ $staff->role }}')">
                        <div class="avatar-wrapper">
                            <img src="{{ $staff->image ? asset('storage/'.$staff->image) : 'https://ui-avatars.com/api/?name='.urlencode($staff->name).'&background=eee&color=333' }}" 
        ...
        <div class="chat-area">
            <div class="chat-top-bar">
                <div class="d-flex align-items-center gap-3" style="cursor:pointer" onclick="viewProfile()">
                    <div class="avatar-wrapper">
                        <img id="chatAvatar" src="https://ui-avatars.com/api/?name=Select+Contact&background=eee" class="contact-avatar">
                    </div>
                    <div>
...
function filterContacts() {
    ...
    if (input.length > 0) container.classList.add('searching');
    else container.classList.remove('searching');

    contacts.forEach(item => {
        let name = item.querySelector('strong').innerText.toLowerCase();
        if (input.length > 0) item.style.setProperty('display', name.includes(input) ? 'flex' : 'none', 'important');
        else item.style.removeProperty('display');
    });
}

/**
 * FIXED: selectContact now fetches profile and updates header
 */
function selectContact(id, type) {
    ...
    // Mark as read
    fetch(`/seller/chat/mark-read/${id}/${type}`, {
        method: 'POST',
        headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }
    });

    // Update Header Profile Info
    fetch(`/seller/chat/profile/${type}/${id}`).then(r => r.json()).then(data => {
        activeUser = data;
...
function loadMessages() {
    ...
    fetch(`/seller/chat/fetch/${currentReceiverId}/${currentReceiverType}`).then(r => r.json()).then(data => {
...
function openOrderModal() {
    fetch("{{ route('seller.chat.orders') }}").then(r => r.json()).then(orders => {
...
function deleteMsg(msgId, deleteType) {
    Swal.fire({ title: 'Are you sure?', text: "Delete message?", icon: 'warning', showCancelButton: true, confirmColor: '#6b2f2f' }).then(res => {
        if (res.isConfirmed) {
            fetch("{{ route('seller.chat.delete') }}", { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' }, body: JSON.stringify({ id: msgId, type: deleteType }) }).then(() => loadMessages());
        }
    });
}
...
function clearChat() {
    ...
    .then(res => { if(res.isConfirmed) { fetch(`/seller/chat/clear/${currentReceiverId}/${currentReceiverType}`, { method:'POST', headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}'} }).then(() => loadMessages()); } });
}
...
function sendMessage(msg, file = null) {
    ...
    fetch("{{ route('seller.chat.send') }}", { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' }, body: formData }).then(() => {
